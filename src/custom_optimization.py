...
def create_optimizer(loss, init_lr, num_train_steps, num_warmup_steps, fp16=False):
    ...
    if fp16:
        all_finite = tf.reduce_all(input_tensor=[tf.reduce_all(input_tensor=tf.math.is_finite(g)) for g in grads])
    else:
    ...
    new_global_step = tf.cond(pred=all_finite, true_fn=lambda: global_step + 1, false_fn=lambda: global_step)
